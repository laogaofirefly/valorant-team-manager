...
router.get('/', async (req, res) => {
  try {
    ...
    errorResponse(res, '获取存档列表失败', 500, (error as Error).message);
  }
});
...
router.get('/:slot', async (req, res) => {
  try {
    ...
    errorResponse(res, '获取存档失败', 500, (error as Error).message);
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    const saveData = {
      ...
      slot: validated.slot,
      name: validated.name,
      gameState: validated.gameState as unknown as GameState,
      isAutoSave: validated.isAutoSave || false,
    };

    // 使用 upsert 更新或创建
    const save = await Save.findOneAndUpdate(
      { userId, slot: validated.slot },
      saveData,
    ...
    errorResponse(res, '保存失败', 500, (error as Error).message);
  }
});
...
router.delete('/:slot', async (req, res) => {
  try {
    ...
    errorResponse(res, '删除存档失败', 500, (error as Error).message);
  }
});
...
router.post('/auto', async (req, res) => {
  try {
    ...
    errorResponse(res, '自动保存失败', 500, (error as Error).message);
  }
});
