...
// 计算比赛结果（AI对战）
const calculateMatchResult = (team1, team2) => {
    ...
    let team2Score = 0;
    for (let i = 0; i < 13; i++) {
        if (Math.random() < power1 / (power1 + power2 + 10))
            team1Score++;
        else
            team2Score++;
    }
...
router.post('/', async (req, res) => {
    try {
        ...
        errorResponse(res, '创建对战失败', 500, error.message);
    }
});
// 接受对战
router.post('/:matchId/accept', async (req, res) => {
    try {
        ...
        errorResponse(res, '接受对战失败', 500, error.message);
    }
});
// 开始对战并计算结果（AI模拟）
router.post('/:matchId/start', async (req, res) => {
    try {
        ...
        errorResponse(res, '开始对战失败', 500, error.message);
    }
});
// 获取对战详情
router.get('/:matchId', async (req, res) => {
    try {
        ...
        errorResponse(res, '获取对战详情失败', 500, error.message);
    }
});
// 获取我的对战列表
router.get('/', async (req, res) => {
    try {
        ...
        errorResponse(res, '获取对战列表失败', 500, error.message);
    }
});
// 主动认输
router.post('/:matchId/forfeit', async (req, res) => {
    try {
        ...
        errorResponse(res, '认输失败', 500, error.message);
    }
});
